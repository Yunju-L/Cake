...
void oven_complete(ObjectPtr oven, ObjectPtr cake, ObjectPtr degree, ObjectPtr time, ObjectPtr next) {
	showMessage("완료!");
	oven->hide();
	degree->hide();
	time->hide();
...
int main() {
	...
	auto sound = Sound::create("Audio/English Country Garden - Aaron Kenny.mp3");
	...
	auto oven = Object::create("Images/오븐.png", scene2, 390, 50, false);
	auto degree = Object::create("Images/버튼.png", scene2, 500, 410, false);
	auto time = Object::create("Images/버튼.png", scene2, 630, 410, false);
	...
	auto deco_choco_ = Object::create("Images/초코.png", scene2, 370, 60, false);
	...
	auto result= Object::create("Images/결과_성공.png", scene2, 100, 530, false);
	auto end= Object::create("Images/끝내기.png", scene2, 1000, 300, false);
	...
	phone->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		if (message_check == 0) {
			showMessage("헉! 케이크를 직접 만들어서 선물해 주기로 한 걸 까맣게 잊고 있었다!\n약속된 시간 전에 얼른 케이크를 만들어야 해!");
			message_check++;
	...
	message->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		...
		message->hide();
		if (message_check == 1) {
			showMessage("흠.. 그런데 난 케이크 만드는 법을 모르는 걸? 이걸 어쩌지?");
			message_check++;
		}
	...
	scene2->setOnEnterCallback([&](ScenePtr scene)->bool {
		...
		else if (message_check != 0 && recipe1->isHanded()) {
	...
	degree->setOnKeypadCallback([&](ObjectPtr object)->bool {
		degree_check=1;
		if (degree_check==1&&time_check==1) oven_complete(oven, cake, degree, time, next);
	...
	cake->setOnMouseCallback([&](ObjectPtr object, int x, int y, MouseAction action)->bool {
		if (stage == 7) {
			result->show();
			end->show();
			if (chili_state == 1) result->setImage("Images/결과_매움.png");
	...
	//***********************************공통***********************************//
		...
		if (stage == 2) {
			...
			next->setImage("Images/결과 확인.png");
			next->show();
		}
		done->hide();
